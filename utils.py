...
def read_parquet(path: str, size: int = None) -> pd.DataFrame:
    if size:
...
def to_normalize_coords(df):
    ...
    if 'coord_adjusted' not in df.columns or not df['coord_adjusted'].any():
        df['x'] = ((df['x'] - x_range[0]) / (x_range[1] - x_range[0])) * (x_target[1] - x_target[0]) + x_target[0]
        df['y'] = ((df['y'] - y_range[0]) / (y_range[1] - y_range[0])) * (y_target[1] - y_target[0]) + y_target[0]

    df['coord_adjusted'] = True
